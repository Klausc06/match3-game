...
```text
match3-game/
├── index.html                    # 入口（主菜单 / 游戏 / 结算 / 排行榜）
├── manifest.webmanifest          # PWA 清单
├── CHANGELOG.md                  # 变更日志（唯一副本，在根目录）
├── css/
│   ├── style.css                 # 全局基础样式
│   ├── game.css                  # 游戏界面 + 图例面板样式
│   └── themes.css                # 花园/家园主题 CSS Variables
├── assets/tiles/                 # ⚠️ 所有 UI 图例必须引用此处素材
│   ├── garden/                   # 花园主题图块 (apple/drop/flower/grass/leaf/pear + dynamite/tnt)
│   ├── home/                     # 家园主题图块 (book/bowtie/cup/cushion/lamp/vase + box/chain/jelly/paperplane)
│   └── shared/                   # 共享素材 (bomb/firecracker/ice/rocket)
├── js/
│   ├── main.js                   # 入口：组装模块 + Demo 按钮
│   ├── config/
│   │   ├── Events.js             # 事件名常量
│   │   ├── GameConfig.js         # 全局游戏参数
│   │   ├── LevelGenerator.js     # 程序化关卡生成器
│   │   └── PowerUpTypes.js       # 道具/障碍物类型常量 (PU/OB)
│   ├── core/
│   │   ├── EventBus.js           # 全局事件总线
│   │   ├── StateMachine.js       # 游戏状态机
│   │   ├── Board.js              # 棋盘数据管理（委托 board/ 子模块）
│   │   ├── board/
│   │   │   ├── matching.js       # 匹配检测 + 道具生成判定
│   │   │   ├── destruction.js    # 消除执行 + 道具引爆
│   │   │   ├── grid.js           # 掉落 + 填充
│   │   │   └── obstacles.js      # 障碍物处理
│   │   ├── GameLoop.js           # requestAnimationFrame 主循环
│   │   ├── InputHandler.js       # 触摸/鼠标输入处理
│   │   ├── Renderer.js           # Canvas 绘制层
│   │   ├── AnimationSystem.js    # 动画队列管理
│   │   ├── AssetLoader.js        # PNG 贴图预加载
│   │   ├── Random.js             # 可种子化随机数
...
│   ├── powerups/
│   │   ├── PowerUp.js            # 道具基类
│   │   ├── PowerUpFactory.js     # 工厂：按类型创建道具实例
...
│   │   ├── Tnt.js                # TNT（花园）
│   │   ├── Rocket.js             # 火箭（家园）
│   │   ├── PaperPlane.js         # 纸飞机（家园）
│   │   └── RainbowBall.js        # 彩虹球（家园，已定义但未在匹配中触发）
│   └── ui/
│       └── UIManager.js          # DOM UI 刷新（分数/时间）
├── docs/
│   ├── api-reference.md          # 📌 所有事件名/接口定义
│   ├── architecture.md           # 架构决策记录
│   ├── ROADMAP.md                # 未来开发计划
│   └── contracts/
│       └── game-contract.v1.json # 关卡配置 schema
├── tests/
│   ├── unit/                     # 单元测试 (board.test / game-loop.test)
│   ├── browser-automation-test.mjs
│   └── browser-automation-assert.mjs
├── tools/
│   ├── validate-contract.mjs     # 关卡配置校验
│   └── validate-level-solvability.mjs
└── .agents/
    ├── skills/
    │   ├── project-rules/        # 📌 本文件 — 开始编码前必读
    │   ├── architecture/         # 模块接口/事件规范 — 新增模块或修改接口前必读
    │   └── canvas-renderer/      # Canvas 绘制规范 — 修改 Renderer.js 前必读
    └── workflows/
        ├── pre-step-check.md     # /pre-step-check — 开发新 Step 前的检查流程
        └── test-module.md        # /test-module — 模块测试清单
```
...
1. **强制留下任务日志 (Task Logging)**
   - 每次重构、新增功能或完成一个 Step 后，必须更新 `CHANGELOG.md`（项目根目录）。
...
4. **素材一致性规则 (Asset Consistency)**
   - UI 图例、文档截图、任何可视化说明中展示的游戏元素图标，**必须使用 `assets/tiles/` 中与游戏 Canvas 渲染相同的 PNG 贴图**，禁止用 emoji 或 AI 生成图片替代。
   - 对于没有独立 PNG 贴图的元素（如地毯、溪流），使用与 `Renderer.js` 中完全一致的 CSS 颜色/效果模拟。
   - 图例中只展示游戏中**实际存在且可见**的元素，不展示未实现或纯代码定义但无法在游戏中出现的功能。

---

## 八、AI 助手语言风格要求 (AI Persona & Tone)

1. **直接客观**：沟通和描述时保持准确、专业、简洁。
2. **摒弃夸张语句**：绝对不要使用诸如"这绝对能给面试官一记震撼的绝杀"、"大功告成！"、"完美的绝杀"之类主观、夸耀性、情绪化的口语。
3. **事实导向**：实事求是陈述改动和方案，展示专业素养和行业经验即可。
